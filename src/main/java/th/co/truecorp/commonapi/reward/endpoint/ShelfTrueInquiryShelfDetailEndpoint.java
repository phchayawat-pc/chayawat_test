...
@Component
public class ShelfTrueInquiryShelfDetailEndpoint {

    private static final Logger log = LoggerFactory.getLogger(ShelfTrueInquiryShelfDetailEndpoint.class);
    ...
    public EndpointResult getShelfInquiryShelfDetail(Map<String, Object> tv) throws Exception {
        ...
        try {
            List<ShelfContentDataApiRsp.ContentData.DataDetails> shelfItemsList = new ArrayList<>();

            log.info("Call GetShelfInquiryShelfDetail");
            ...
            ResponseEntity<ProgressiveShelvesRsp> gwResponse = apigwService.execute(
                    logContext,
                    HttpMethod.GET,
                    Constant.ENDPOINT_SOURCE_SYSTEM_ID,
                    Constant.ENDPOINT_SERVICE_GET_INQUIRY_SHELF_DETAIL,
                    ProgressiveShelvesRsp.class,
                    tv,
                    headers,
                    pathParams,
                    queryParams
            ...
            if (gwResponse != null && !gwResponse.getStatusCode().is2xxSuccessful()) {
                ...
                tv.put(Constant.ENDPOINT_RESULT_RWD,endpointResult2);
                shelfItemsList = parseShelfResponse(Objects.requireNonNull(gwResponse.getBody()).getProgressive_shelves().getData().getShelf_items(), tv);
            }
            tv.put("GetShelfInquiryShelfDetail", shelfItemsList);
    ...
    private Map<String, Object> createQueryParams(Map<String, Object> tv) {
        ...
        queryParams.put("lang", Objects.equals(tv.get("lang").toString(), Constant.TH) ? "T" : "E");
        queryParams.put("limit", tv.get("limit"));
        queryParams.put("fields", tv.get("fields"));
        queryParams.put("cms_id", tv.get("cms_id"));
        queryParams.put("channel", tv.get("channel"));
        return queryParams;
    }

    private List<ShelfContentDataApiRsp.ContentData.DataDetails> parseShelfResponse(List<ProgressiveShelvesRsp.ShelfItem> shelfItemsListNode, Map<String, Object> tv) throws Exception {
        List<ShelfContentDataApiRsp.ContentData.DataDetails> shelfItemsList = new ArrayList<>();

        log.debug("map response value from api...");
        for (ProgressiveShelvesRsp.ShelfItem result : shelfItemsListNode) {
            ...
            details.setDisplay_lang(result.getLang());
            ...
            details.setRedeem_point(String.valueOf(result.getRedeem_point()));
            ...
            details.setItem_type(result.getItem_type());

            shelfItemsList.add(details);
        }
        log.debug("map response value from api success");
        return shelfItemsList;
    }

    private List<String> mapListString(ArrayList<String> jn) {
    ...
    private ShelfContentDataApiRsp.ThumbList mapThumbList(ProgressiveShelvesRsp.ThumbList jsonNode) {
    ...
    private ShelfContentDataApiRsp.InfoData mapInfo(ProgressiveShelvesRsp.Info jsonNode) {
    ...
    private ShelfContentDataApiRsp.SettingData mapSetting(ProgressiveShelvesRsp.Setting jsonNode) {
